...
using Plots, PlutoUI, LaTeXStrings, LinearAlgebra, DifferentialEquations

# ╔═╡ a97d10c0-f4c5-40f2-832b-95e5d0944091
md"# Vector fields

Let's try to plot a simple vector field:
...
"For a point ``($(x0), $(y0))``
our derivates will correspond to:
...
begin	
	...
	for _ in 1:ts
		...
		quiver!([x], [y], quiver=([Δx],[Δy]), c=:tomato, linewidth=1.5)
	end
	p
end
...
begin
	space = -3:0.25:3
	xs = [x for x in space for y in space]
	ys = [y for x in space for y in space]
	dxs = dx.(ys,0.1)
	dys = dy.(xs,0.1)
	p2 = plot(size=(500,500), leg=false)
...
begin
	p3 = plot(size=(500,500), leg=false, background=:black)
	norms = norm.(zip(dxs,dys)) * 6
	dxs_norm = dxs ./ norms
	dys_norm = dys ./ norms
...
git-tree-sha1 = "d5cbff8f4872adddb8c6a597ef5b2da25137ba98"
...
git-tree-sha1 = "7044adcf5ee24a6b3c26c086ab65cd6366c8d943"
