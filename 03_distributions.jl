...
# ╔═╡ ac78d99e-fb0b-11eb-2c7c-adbf03728233
begin
	using PlutoUI, Random, Distributions, Plots, LaTeXStrings
	
	format_number(x,d) = round(x, digits=d)
	format_number(x) = format_number(x,4)
	
	PlutoUI.TableOfContents()
end

# ╔═╡ 0e8109de-a61c-468e-a67e-7f593a2ae8c8
md"# Exploration of statistical distributions"
...
-  $x$ is the number of succesful trials.
...
begin
	xs = 0:n
	p1 = plot(xs, pdf.(dist_binomial, xs), label="PDF",
		title="Distribution as x changes (p=$p)",
	...
	plot!(ps, map(p -> cdf(Binomial(n,p), x), ps), label="CDF")
	scatter!([p], [pdf(Binomial(n,p), x)], label=false)
	plot(p1, p2, layout=(2,1))
end

# ╔═╡ 4fc91165-28ea-41f6-86aa-89ba1453de68
@bind go Button("Random sample")

# ╔═╡ 3ac94b0a-39ef-413b-b1c6-beea66f401a0
begin
	go
	histogram(rand(dist_binomial, 1000), label=false, size=(600,150),
		title="Histogram of 1000 samples from the distribution")
	vline!([mean(dist_binomial)], linewidth=2, linestyle=:dash, label=L"E(P_x)")
end

# ╔═╡ 52c3c8bf-e53d-4617-a825-f57615a7168b
md"## Poisson distribution
>The idea of the poisson distribution is to predict the probability of a given number of events occurring in a fixed interval of time.
"

# ╔═╡ 78ceddb5-04e3-492a-b3a7-78138dfc02c0
md"The probability of observing $k$ events in an interval in given by:

$P(k) = e^{-λ} \frac{λ^k}{k!}$
Where:
-  $P$ is the poisson probability.
-  $λ$ is the average number of events per interval of time (rate).
-  $k$ is the number of events to observe.
"
...
begin
	dist_poisson = Poisson(λ)
	
	Markdown.parse("\$\$P($k) = e^{-$λ} \\frac{$λ^{$k}}{$(k)!} = $(pdf(dist_poisson, k) |> format_number)\$\$")
end

# ╔═╡ 765bbf8b-00c7-44f7-a3e2-f214400123a6
begin
	ks = 0:25
	p_poisson_1 = plot(ks, pdf.(dist_poisson, ks), label="PDF",
		title="Distribution as k changes (λ=$λ)",
		xlabel=L"k", legend=:topleft)
	plot!(ks, cdf.(dist_poisson, ks), label="CDF")
	scatter!([k], [pdf(dist_poisson,k)], label=false)
	
	λs = 0:25
	p_poisson_2 = plot(λs, map(λ -> pdf(Poisson(λ), k), λs), label="PDF",
		title="Distribution as λ changes (k=$k)",
		xlabel=L"\lambda", legend=:topright)
	plot!(λs, map(λ -> cdf.(Poisson(λ), k), λs), label="CDF")
	scatter!([λ], [pdf(Poisson(λ), k)], label=false)
	
	plot(p_poisson_1, p_poisson_2, layout=(2,1))
end

# ╔═╡ 02d255a9-1e39-4412-a402-645d551afe14
@bind go_poisson Button("Do sample")

# ╔═╡ b254beba-2119-428e-8d30-7aad3697e957
begin
	go_poisson
	histogram(rand(dist_poisson, 1000), label=false, size=(600,150),
		title="Histogram of 1000 samples from the distribution")
	vline!([mean(dist_poisson)], linewidth=2, linestyle=:dash, label=L"E(P_k)")
end
...
begin
	xs_exp = 0:0.2:10
	p_exp_1 = plot(xs_exp, pdf.(dist_exp, xs_exp), label="PDF",
...
@bind go_exp Button("Do sample")

# ╔═╡ 11c2337b-c1e6-4dd5-a5a6-6343747bb980
begin
	go_exp
	histogram(rand(dist_exp, 1000), label=false, size=(600,150),
		title="Histogram of 1000 samples from the distribution")
	vline!([mean(dist_exp)], linewidth=2, linestyle=:dash, label=L"E[f(x;\theta)]")
end
